...
const Index = function ({user}) {
  ...
  const getTours = async () => {
      ...
      console.log(toursResp);
      if(toursResp.status === 200) setTours(toursResp.data);
  };

  const deleteTour = async tour => {
    try{
      const toursResp = await Axios.post('/api/tours/delete',{
        id: tour._id
      });
      if(toursResp.status === 200){
        toast("Tour deleted succesfully!", {type: toast.TYPE.SUCCESS});
        await getTours();
      }
    }catch(error){
      toast("Thats weird, it didn't work!", {type: toast.TYPE.ERROR});
    }
  }
  return (
